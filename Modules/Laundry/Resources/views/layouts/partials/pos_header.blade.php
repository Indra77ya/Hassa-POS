@if(!empty($__is_laundry_enabled))
    @if(auth()->check() && (auth()->user()->can('superadmin') || auth()->user()->can('laundry.create') || auth()->user()->can('laundry.view')))
        ...
        href="{{ action([\App\Http\Controllers\SellPosController::class, 'create']). '?sub_type=laundry'}}" title="{{ __('laundry::lang.add_laundry_order') }}" data-toggle="tooltip" data-placement="bottom">
