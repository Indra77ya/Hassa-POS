                ...
                <tbody>
                    @forelse($recent_orders as $order)
                        <tr>
                            ...
                            <td>{{ number_format($order->quantity, session('business.quantity_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']) }} {{ $order->unit_name }}</td>
                            <td>
                                @if($order->status)
