...
<div class="col-md-12 no-print pos-header" style="padding: 6px 12px; background-color: #ecf0f5; border-bottom: 1px solid #d2d6de;">
    <input type="hidden" id="pos_redirect_url" value="{{ $pos_redirect_url }}">
    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 4px;">

        <!-- Left side: Location & Datetime -->
        <div style="display: flex; align-items: center; gap: 4px;">
            <span class="btn btn-primary btn-sm btn-flat" style="cursor: default; font-weight: bold; padding: 4px 10px; height: 30px; display: inline-flex; align-items: center;">
                ...
                @if (empty($transaction->location_id))
                    @if (count($business_locations) > 1)
                        {!! Form::select(
                            ...
                            ['class' => 'input-sm', 'id' => 'select_location_id', 'required', 'autofocus', 'style' => 'background: transparent; color: #fff; border: none; outline: none; font-weight: bold; padding: 0; margin: 0; height: auto;'],
                            $bl_attributes,
                        ) !!}
                    @else
            ...
            <span class="btn btn-default btn-sm btn-flat hidden-xs" style="cursor: default; font-weight: bold; padding: 4px 10px; height: 30px; display: inline-flex; align-items: center; background-color: #fff; border-color: #ccc;">
                <i class="fa fa-clock-o text-muted"></i> &nbsp;<span class="curr_datetime">{{ @format_datetime('now') }}</span>
                <i class="fa fa-keyboard-o hover-q text-muted" aria-hidden="true" data-container="body"
                    ...
                    data-html="true" data-trigger="hover" data-original-title="" title="" style="margin-left: 6px; cursor: pointer;"></i>
            </span>

            @if (empty($pos_settings['hide_product_suggestion']))
                <button type="button" title="{{ __('lang_v1.view_products') }}" data-placement="bottom"
                    class="btn btn-default btn-sm btn-flat visible-xs btn-modal"
        ...
        <div style="display: flex; align-items: center; justify-content: flex-end; flex-wrap: wrap; gap: 4px;">
            <a href="{{ $go_back_url }}" title="{{ __('lang_v1.go_back') }}" class="btn btn-info btn-flat btn-sm" style="font-weight: bold; height: 30px; display: inline-flex; align-items: center;">
                <i class="fa fa-backward"></i> &nbsp;<span class="hidden-xs">{{ __('lang_v1.go_back') }}</span>
            </a>

            @if (!isset($pos_settings['hide_recent_trans']) || $pos_settings['hide_recent_trans'] == 0)
                <button type="button" class="btn btn-primary btn-flat btn-sm visible-xs-inline-block" data-toggle="modal" data-target="#recent_transactions_modal" id="recent-transactions" style="height: 30px;">
            ...
            @if (!empty($pos_settings['inline_service_staff']))
                <button type="button" id="show_service_staff_availability" title="{{ __('lang_v1.service_staff_availability') }}"
                    class="btn btn-default btn-flat btn-sm" data-container=".view_modal" style="height: 30px;"
            ...
            @can('close_cash_register')
                <button type="button" id="close_register" title="{{ __('cash_register.close_register') }}"
                    class="btn btn-danger btn-flat btn-sm btn-modal" data-container=".close_register_modal" style="font-weight: bold; height: 30px; display: inline-flex; align-items: center;"
                    data-href="{{ action([\App\Http\Controllers\CashRegisterController::class, 'getCloseRegister']) }}">
                    <i class="fa fa-window-close"></i> &nbsp;<span class="hidden-xs">{{ __('cash_register.close_register') }}</span>
                </button>
            @endcan

            @if (!empty($pos_settings['inline_service_staff']) || (in_array('tables', $enabled_modules) || in_array('service_staff', $enabled_modules)))
                <button type="button" class="btn btn-default btn-flat btn-sm popover-default" style="height: 30px;"
            ...
            @can('view_cash_register')
                <button type="button" id="register_details" title="{{ __('cash_register.register_details') }}"
                    class="btn btn-success btn-flat btn-sm btn-modal" data-container=".register_details_modal" style="font-weight: bold; height: 30px; display: inline-flex; align-items: center;"
                    data-href="{{ action([\App\Http\Controllers\CashRegisterController::class, 'getRegisterDetails']) }}">
                    <i class="fa fa-briefcase"></i> &nbsp;<span class="hidden-xs">{{ __('cash_register.register_details') }}</span>
                </button>
            @endcan

            <button title="@lang('lang_v1.calculator')" id="btnCalculator" type="button" class="btn btn-default btn-flat btn-sm popover-default" style="height: 30px;"
                ...
                <i class="fa fa-calculator text-teal"></i>
            </button>

            <button type="button" class="btn btn-warning btn-flat btn-sm popover-default" style="font-weight: bold; height: 30px; display: inline-flex; align-items: center;"
                ...
                data-html="true" data-placement="bottom">
                <i class="fa fa-undo"></i> &nbsp;<span class="hidden-xs">@lang('lang_v1.sell_return')</span>
            </button>

            <button type="button" id="toggle-dark-mode" title="Toggle Dark Mode" class="btn btn-default btn-flat btn-sm" style="height: 30px;">
                <i class="fa fa-moon-o" id="dark-mode-icon-moon"></i>
                <i class="fa fa-sun-o hide" id="dark-mode-icon-sun"></i>
            </button>

            <button type="button" title="{{ __('lang_v1.full_screen') }}" class="btn btn-default btn-flat btn-sm" id="full_screen" style="height: 30px;">
                <i class="fa fa-window-maximize"></i>
            </button>

            <button type="button" id="view_suspended_sales" title="{{ __('lang_v1.view_suspended_sales') }}"
                class="btn btn-warning btn-flat btn-sm btn-modal" data-container=".view_modal" data-href="{{ $view_suspended_sell_url }}" style="font-weight: bold; height: 30px; display: inline-flex; align-items: center;">
                <i class="fa fa-pause-circle"></i> &nbsp;<span class="hidden-xs">{{ __('lang_v1.view_suspended_sales') }}</span>
            </button>
            ...
            @if (!empty($pos_settings['customer_display_screen']))
                <a href="{{route('pos_display')}}" id="customer_display_screen" onclick="window.open(this.href, 'customer_display', 'width='+screen.width+',height='+screen.height+',top=0,left=0'); return false;" title="{{ __('lang_v1.customer_display_screen') }}"
                    class="btn btn-default btn-flat btn-sm" style="height: 30px; display: inline-flex; align-items: center;">
                    <i class="fa fa-tv text-blue"></i>
                </a>
            @endif
            ...
            @if (in_array('pos_sale', $enabled_modules) && !empty($transaction_sub_type))
                @can('sell.create')
                    <a href="{{ action([\App\Http\Controllers\SellPosController::class, 'create']) }}"
                        title="@lang('sale.pos_sale')" class="btn btn-success btn-flat btn-sm" style="font-weight: bold; height: 30px; display: inline-flex; align-items: center;">
            ...
            @can('expense.add')
                <button type="button" title="{{ __('expense.add_expense') }}" data-placement="bottom"
                    class="btn btn-danger btn-flat btn-sm btn-modal" id="add_expense" style="font-weight: bold; height: 30px; display: inline-flex; align-items: center;">
                    <i class="fa fa-minus-circle"></i> &nbsp;@lang('expense.add_expense')
                </button>
            @endcan
        </div>
    </div>
