<div class="pos-sidebar-root tw-shadow-[rgba(15,23,42,0.04)_0px_3px_12px] tw-rounded-2xl tw-bg-white" style="padding: 6px; overflow: hidden; height: 100%;">
<div class="tw-flex tw-items-start tw-gap-2 tw-flex-wrap" style="margin: 0 0 6px 0; padding: 0 4px;">
            ...
            <div class="tw-dw-drawer tw-dw-drawer-end">
                ...
                <div class="tw-dw-drawer-content">
                    ...
                    <label for="my-drawer-4{{ $drawer_id_suffix ?? '' }}"
                        class="tw-dw-btn tw-dw-btn-sm tw-group tw-w-full tw-h-9 tw-min-h-[2.25rem] tw-rounded-xl tw-flex tw-flex-row tw-items-center tw-flex-nowrap tw-gap-1.5 tw-px-3 tw-text-sm tw-font-semibold tw-normal-case tw-bg-white tw-border-slate-200 tw-text-slate-600 tw-shadow-sm tw-transition-all tw-duration-150 hover:tw-bg-slate-50 hover:tw-border-slate-300 hover:tw-text-slate-800 active:tw-scale-[0.97] focus:tw-outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="tw-w-4 md:tw-w-5 tw-flex-shrink-0 tw-text-slate-400 tw-transition-transform tw-duration-200 group-hover:tw-scale-110 icon icon-tabler icon-tabler-category-plus" width="44" height="44"
                        ...
                        <span class="pos-active-filter-badge pos-filter-badge-cat tw-dw-badge tw-dw-badge-md tw-bg-slate-50 tw-border tw-border-slate-200 tw-text-slate-600 tw-gap-1 tw-max-w-[110px] tw-flex-shrink-0 tw-font-semibold tw-text-[12px] tw-px-2">
                            <span class="pos-active-filter-name tw-truncate"></span>
                            <button type="button" class="pos-filter-chip-clear tw-text-slate-400" data-clear="category" aria-label="Clear">
                        ...
                        <span class="pos-count-badge tw-dw-badge tw-dw-badge-sm tw-bg-slate-50 tw-border-slate-200 tw-text-slate-500 tw-font-bold tw-text-[11px] tw-flex-shrink-0 group-hover:tw-bg-slate-100 group-hover:tw-border-slate-300 tw-transition-colors">{{ count($categories) }}</span>
                    </label>
                </div>
                    ...
                    <div class="tw-dw-menu pos-drawer-panel pos-drawer-category tw-min-h-full tw-bg-white">
                        ...
                        <div class="tw-flex tw-items-start tw-justify-between tw-gap-3 tw-px-6 tw-pt-[22px] tw-pb-4 tw-border-b tw-border-slate-100">
                            <div class="tw-flex-1 tw-min-w-0">
                                ...
                                <div class="tw-font-mono tw-text-[11px] tw-tracking-[0.08em] tw-uppercase tw-text-slate-400 tw-mt-1.5"><span>{{ count($categories) }}</span> @lang('category.category')</div>
                            </div>
                            <button type="button" class="tw-w-9 tw-h-9 tw-rounded-[10px] tw-bg-slate-100 tw-text-slate-500 tw-border-0 tw-cursor-pointer tw-inline-flex tw-items-center tw-justify-center tw-transition-all tw-duration-[160ms] tw-flex-shrink-0 hover:tw-bg-slate-200 hover:tw-text-slate-900 focus-visible:tw-outline focus-visible:tw-outline-2 focus-visible:tw-outline-slate-500 focus-visible:tw-outline-offset-2 close-side-bar-category" aria-label="Close">
                        ...
                        <div class="pos-drawer-body tw-px-6 tw-pt-[18px] tw-pb-6 tw-overflow-y-auto tw-flex-1">
                            <div class="row pos-card-grid pos-card-grid-categories" data-label-all="@lang('messages.all')" style="margin-right: 0; margin-left: -8px;">
                                <div class="col-md-4 col-xs-6 tw-mb-3 tw-cursor-pointer main-category-div main-category no-print"
                                    ...
                                    <div class="pos-card cat tw-rounded-xl tw-border tw-border-slate-100 hover:tw-border-slate-300 tw-transition-all">
                                        <h4 class="pos-card-name tw-font-semibold tw-text-slate-700">@lang('lang_v1.all_category')</h4>
                                    </div>
                                </div>
                                @foreach ($categories as $category)
                                    <div class="col-md-4 col-xs-6 tw-mb-3 tw-cursor-pointer main-category-div main-category no-print"
                                        ...
                                        <div class="pos-card cat tw-rounded-xl tw-border tw-border-slate-100 hover:tw-border-slate-300 tw-transition-all">
                                            <h4 class="pos-card-name tw-font-semibold tw-text-slate-700">{{ $category['name'] }}</h4>
                                            ...
                                            @foreach ($category['sub_categories'] as $sc)
                                                @if ($sc['parent_id'] != 0)
                                                    <div class="col-md-4 col-xs-6 tw-mb-3 tw-cursor-pointer product_category no-print"
                                                        ...
                                                        style="padding-left: 4px; padding-right: 4px;">
                                                        <div class="pos-card cat tw-rounded-xl tw-border tw-border-slate-100 hover:tw-border-slate-300 tw-transition-all">
                                                            <h4 class="pos-card-name tw-font-semibold tw-text-slate-700">{{ $sc['name'] }}</h4>
                                                        </div>
                                                    </div>
                                                @endif
                                            @endforeach
            ...
            <div class="tw-dw-drawer tw-dw-drawer-end">
                ...
                <div class="tw-dw-drawer-content">
                    ...
                    <label for="my-drawer-brand{{ $drawer_id_suffix ?? '' }}"
                        class="tw-dw-btn tw-dw-btn-sm tw-group tw-w-full tw-h-9 tw-min-h-[2.25rem] tw-rounded-xl tw-flex tw-flex-row tw-items-center tw-flex-nowrap tw-gap-1.5 tw-px-3 tw-text-sm tw-font-semibold tw-normal-case tw-bg-white tw-border-slate-200 tw-text-slate-600 tw-shadow-sm tw-transition-all tw-duration-150 hover:tw-bg-slate-50 hover:tw-border-slate-300 hover:tw-text-slate-800 active:tw-scale-[0.97] focus:tw-outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg" class="tw-w-4 md:tw-w-5 tw-flex-shrink-0 tw-text-slate-400 tw-transition-transform tw-duration-200 group-hover:tw-scale-110 icon icon-tabler icon-tabler-brand-beats"
                        ...
                        <span class="pos-active-filter-badge pos-filter-badge-brand tw-dw-badge tw-dw-badge-md tw-bg-slate-50 tw-border tw-border-slate-200 tw-text-slate-600 tw-gap-1 tw-max-w-[110px] tw-flex-shrink-0 tw-font-semibold tw-text-[12px] tw-px-2">
                            <span class="pos-active-filter-name tw-truncate"></span>
                            <button type="button" class="pos-filter-chip-clear tw-text-slate-400" data-clear="brand" aria-label="Clear">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
                            </button>
                        </span>
                        {{-- Count badge: hidden when filter is active --}}
                        <span class="pos-count-badge tw-dw-badge tw-dw-badge-sm tw-bg-slate-50 tw-border-slate-200 tw-text-slate-500 tw-font-bold tw-text-[11px] tw-flex-shrink-0 group-hover:tw-bg-slate-100 group-hover:tw-border-slate-300 tw-transition-colors">{{ count($brands) }}</span>
                    </label>
                </div>
                <div class="tw-dw-drawer-side" style="z-index: 4000">
                    ...
                    <div class="tw-dw-menu pos-drawer-panel pos-drawer-brand tw-min-h-full tw-bg-white">
                        ...
                        <div class="tw-flex tw-items-start tw-justify-between tw-gap-3 tw-px-6 tw-pt-[22px] tw-pb-4 tw-border-b tw-border-slate-100">
                            <div class="tw-flex-1 tw-min-w-0">
                                ...
                                <div class="tw-font-mono tw-text-[11px] tw-tracking-[0.08em] tw-uppercase tw-text-slate-400 tw-mt-1.5"><span>{{ count($brands) }}</span> @lang('brand.brands')</div>
                            </div>
                            <button type="button" class="tw-w-9 tw-h-9 tw-rounded-[10px] tw-bg-slate-100 tw-text-slate-500 tw-border-0 tw-cursor-pointer tw-inline-flex tw-items-center tw-justify-center tw-transition-all tw-duration-[160ms] tw-flex-shrink-0 hover:tw-bg-slate-200 hover:tw-text-slate-900 focus-visible:tw-outline focus-visible:tw-outline-2 focus-visible:tw-outline-slate-500 focus-visible:tw-outline-offset-2 close-side-bar-brand" aria-label="Close">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
                            </button>
                        </div>
                            ...
                            <div class="row pos-card-grid pos-card-grid-brands" style="margin-right: 0; margin-left: -8px;">
                                @foreach ($brands as $key => $brand)
                                    <div class="col-md-4 col-xs-6 tw-mb-3 tw-cursor-pointer product_brand no-print"
                                        ...
                                        style="padding-left: 4px; padding-right: 4px;">
                                        <div class="pos-card brand tw-rounded-xl tw-border tw-border-slate-100 hover:tw-border-slate-300 tw-transition-all">
                                            <h4 class="pos-card-name tw-font-semibold tw-text-slate-700">{{ $brand }}</h4>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
    ...
    <div class="tw-flex-1 tw-min-w-[140px]" id="feature_product_div">
        <button type="button" id="show_featured_products"
            ...
            <svg xmlns="http://www.w3.org/2000/svg"
                class="tw-w-4 md:tw-w-5 tw-flex-shrink-0 tw-text-slate-400 tw-transition-transform tw-duration-200 group-hover:tw-scale-110 group-hover:tw-rotate-6 icon icon-tabler icon-tabler-star" width="44" height="44"
                viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" fill="none"
                stroke-linecap="round" stroke-linejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
            ...
            @if (!empty($featured_products) && count($featured_products) > 0)
                <span class="tw-dw-badge tw-dw-badge-sm tw-bg-slate-50 tw-border-slate-200 tw-text-slate-500 tw-font-bold tw-text-[11px] group-hover:tw-bg-slate-100 group-hover:tw-border-slate-300 tw-transition-colors">{{ count($featured_products) }}</span>
            @endif
        </button>
    </div>
</div>
    ...
    <div class="col-md-12" style="padding: 0;">
        <div id="product_list_body" class="eq-height-row tw-max-h-[calc(100vh_-_229px)] tw-overflow-y-auto tw-overflow-x-hidden" style="padding-right: 4px;">
            ...
            <div id="featured_empty_state_template" style="display: none;">
                <div class="tw-w-full tw-flex tw-flex-wrap tw-items-center tw-justify-center tw-gap-x-2 tw-gap-y-1 tw-py-3 tw-px-3 tw-text-slate-600 tw-text-xs md:tw-text-sm">
                    ...
                    <a href="{{ url('business-location') }}" target="_blank" class="tw-inline-flex tw-items-center tw-gap-1 tw-font-semibold tw-text-slate-700 tw-underline tw-decoration-slate-300 tw-underline-offset-2 hover:tw-text-slate-900 hover:tw-decoration-slate-500 tw-transition-colors">
