@forelse($products as $product)
	<div class="col-md-3 col-xs-4 no-print !tw-px-[4px] tw-mb-2">
		<div class="product_box tw-w-full tw-text-center tw-cursor-pointer tw-bg-white tw-p-2 tw-border tw-border-slate-100 tw-shadow-[0_2px_8px_rgba(0,0,0,0.02)] hover:-tw-translate-y-0.5 hover:tw-shadow-[0_6px_20px_rgba(99,102,241,0.08)] active:tw-scale-[0.97] @if($product->enable_stock && $product->qty_available <= 0) product_out_of_stock !tw-bg-slate-50 tw-opacity-65 @endif" data-variation_id="{{$product->id}}" title="{{$product->name}} @if($product->type == 'variable')- {{$product->variation}} @endif {{ '(' . $product->sub_sku . ')'}} @if(!empty($show_prices)) @lang('lang_v1.default') - @format_currency($product->selling_price) @foreach($product->group_prices as $group_price) @if(array_key_exists($group_price->price_group_id, $allowed_group_prices)) {{$allowed_group_prices[$group_price->price_group_id]}} - @format_currency($group_price->price_inc_tax) @endif @endforeach @endif">

			<div class="image-container tw-h-16 tw-w-16 tw-mx-auto tw-mb-2 tw-rounded-lg tw-overflow-hidden tw-bg-slate-50 tw-border tw-border-slate-100/50"
				...
				background-size: {{ count($product->media) > 0 || !empty($product->product_image) ? 'cover' : 'contain' }}; height: 64px; width: 64px;">
			</div>

			<div class="text_div tw-mt-1">
				<div class="tw-text-slate-700 tw-text-[12px] tw-line-clamp-1 tw-leading-tight tw-mb-0.5" style="max-height: 1.25rem;">
					{{$product->name}}
...
@empty
	...
	<div class="col-md-12">
		<div class="tw-flex tw-flex-col tw-items-center tw-justify-center tw-text-center tw-py-12 tw-px-6">
			...
			<div class="tw-text-slate-600 tw-text-sm">@lang('lang_v1.no_products_to_display')</div>
		</div>
	</div>
@endforelse
