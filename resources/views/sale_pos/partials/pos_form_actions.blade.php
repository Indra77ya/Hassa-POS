    ...
    <div
        ...
        <div
            ...
            <div class="!tw-w-full md:!tw-w-none !tw-flex md:!tw-hidden !tw-flex-row !tw-items-center !tw-gap-3">
                @if (empty($edit))
                    <button type="button" class="tw-leading-none tw-whitespace-nowrap tw-text-rose-600 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-bg-rose-50 hover:tw-bg-rose-100 tw-border tw-border-rose-100 tw-p-2.5 tw-w-[5.5rem] tw-flex tw-flex-row tw-items-center tw-justify-center tw-gap-1 active:tw-scale-95 js-pos-cancel"> <i
                ...
                @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <button type="button"
                        class="pos-finalize tw-leading-none tw-whitespace-nowrap tw-flex tw-flex-row tw-items-center tw-justify-center tw-gap-1.5 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-bg-slate-800 hover:tw-bg-slate-900 tw-p-2.5 tw-w-[8.5rem] active:tw-scale-95 no-print @if ($pos_settings['disable_pay_checkout'] != 0) hide @endif"
                ...
                @if (!Gate::check('disable_express_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <button type="button"
                        class="tw-leading-none tw-whitespace-nowrap tw-cursor-pointer tw-text-xs md:tw-text-sm tw-p-2.5 tw-w-[5.5rem] tw-flex tw-flex-row tw-items-center tw-justify-center tw-gap-1 active:tw-scale-95 no-print @if ($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif pos-express-finalize"
            ...
            <div class="tw-flex tw-items-center tw-gap-4 tw-flex-row tw-overflow-x-auto pos-footer-secondary">
                ...
                <div class="!tw-hidden md:!tw-flex md:tw-items-center md:tw-gap-3">
                    @if (empty($edit))
                        <button type="button"
                            class="tw-leading-none tw-whitespace-nowrap tw-text-rose-600 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-bg-rose-50 hover:tw-bg-rose-100 tw-border tw-border-rose-100 tw-py-2 tw-px-4 tw-w-[8.5rem] tw-flex tw-flex-row tw-items-center tw-justify-center tw-gap-1.5 active:tw-scale-95 js-pos-cancel"> <i
                                class="fas fa-window-close"></i> @lang('sale.cancel')</button>
                    @else
                        <button type="button"
                            class="tw-leading-none tw-whitespace-nowrap tw-cursor-pointer tw-text-xs md:tw-text-sm tw-bg-rose-600 hover:tw-bg-rose-700 tw-py-2 tw-px-4 tw-w-[8.5rem] tw-flex tw-flex-row tw-items-center tw-justify-center tw-gap-1.5 hide active:tw-scale-95 js-pos-delete"
                ...
                @if (!Gate::check('disable_draft') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <button type="button"
                        class="tw-text-slate-600 hover:tw-text-indigo-600 tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1 tw-whitespace-nowrap tw-transition-colors @if ($pos_settings['disable_draft'] != 0) hide @endif"
                ...
                @if (!Gate::check('disable_quotation') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <button type="button"
                        class="tw-text-slate-600 hover:tw-text-amber-600 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1 tw-whitespace-nowrap tw-transition-colors @if ($is_mobile) col-xs-6 @endif"
                ...
                @if (!Gate::check('disable_suspend_sale') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    @if (empty($pos_settings['disable_suspend']))
                        <button type="button"
                            class="tw-text-slate-600 hover:tw-text-rose-600 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1 tw-whitespace-nowrap tw-transition-colors no-print pos-express-finalize"
                ...
                @if (!Gate::check('disable_credit_sale') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    @if (empty($pos_settings['disable_credit_sale_button']))
                        ...
                        <button type="button"
                            class="tw-text-slate-600 hover:tw-text-violet-600 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1 tw-whitespace-nowrap tw-transition-colors no-print pos-express-finalize @if ($is_mobile) col-xs-6 @endif"
                ...
                <div class="!tw-hidden md:!tw-flex md:tw-items-center md:tw-gap-3">
                    ...
                    @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                        <button type="button"
                            class="pos-finalize tw-leading-none tw-whitespace-nowrap tw-flex tw-flex-row tw-items-center tw-justify-center tw-gap-1.5 tw-font-extrabold tw-cursor-pointer tw-text-xs md:tw-text-sm tw-bg-slate-800 hover:tw-bg-slate-900 tw-py-2.5 tw-px-5 hover:-tw-translate-y-px active:tw-scale-95 tw-shadow-sm hover:tw-shadow-md no-print @if ($pos_settings['disable_pay_checkout'] != 0) hide @endif"
                            style="width: 150px !important; height: 38px !important;"
                            title="@lang('lang_v1.tooltip_checkout_multi_pay')"><i class="fas fa-money-check-alt"
                                aria-hidden="true"></i> @lang('lang_v1.checkout_multi_pay') </button>
                    @endif
                    ...
                    @if (!Gate::check('disable_express_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                        <button type="button"
                            class="tw-leading-none tw-whitespace-nowrap tw-font-extrabold tw-cursor-pointer tw-text-xs md:tw-text-sm tw-py-2.5 tw-px-5 tw-flex tw-flex-row tw-items-center tw-justify-center tw-gap-1.5 hover:-tw-translate-y-px active:tw-scale-95 tw-shadow-sm hover:tw-shadow-md no-print @if ($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif pos-express-finalize"
                            style="background-color: #10b981 !important; color: #ffffff !important; width: 150px !important; height: 38px !important;"
                            data-pay_method="cash" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt"
                                aria-hidden="true"></i> @lang('lang_v1.express_checkout_cash')</button>
                    @endif
                </div>
            ...
            <div class="tw-w-full md:tw-w-fit tw-flex tw-items-center tw-justify-end tw-hidden md:tw-flex" style="padding-right: 36px !important;">
                @if (!isset($pos_settings['hide_recent_trans']) || $pos_settings['hide_recent_trans'] == 0)
                    <button type="button"
                        class="tw-bg-indigo-50 hover:tw-bg-indigo-100 tw-border tw-border-indigo-100 tw-text-indigo-600 tw-w-full md:tw-w-fit tw-px-4 tw-h-9 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-inline-flex tw-items-center tw-justify-center tw-gap-1.5 hover:-tw-translate-y-px active:tw-scale-95"
