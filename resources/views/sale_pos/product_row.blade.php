...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	@if(!empty($is_serial_no))
		<td class="serial_no text-center" style="vertical-align: middle;"></td>
	@endif
	<td style="vertical-align: middle;">
		...
		<div style="display: flex; align-items: center; gap: 8px;">
			<img src="{{ count($product->media) > 0 ? $product->media->first()->display_url : (!empty($product->product_image) ? asset('/uploads/img/' . rawurlencode($product->product_image)) : asset('/img/default.png')) }}" alt="product-img" loading="lazy" style="width: 35px; height: 35px; border-radius: 3px; object-fit: cover; border: 1px solid #d2d6de;" onerror="this.style.display='none'">
			<div style="min-width: 0; flex: 1;">
				@if( ($edit_price || $edit_discount) && empty($is_direct_sell) )
				<div title="@lang('lang_v1.pos_edit_product_price_help')">
				<span class="text-link text-info cursor-pointer" style="font-weight: bold; font-size: 13px;" data-toggle="modal" data-target="#row_edit_product_price_modal_{{$row_count}}">
					{!! $product_name !!} &nbsp;<i class="fa fa-info-circle" style="font-size: 11px;"></i>
				</span>
				</div>
				@else
					<span style="font-weight: bold; font-size: 13px;">{!! $product_name !!}</span>
				@endif
				<div class="text-muted" style="font-size: 11px;">{{ $product_sku_brand }}@if($product->enable_stock) &nbsp;·&nbsp; <span class="text-primary" style="font-weight: bold;">{{ @num_format($product->qty_available) }} {{$product->unit}}</span>@endif</div>
			</div>
		</div>
	...
	<td style="vertical-align: middle;">
		...
		@foreach($sub_units as $key => $value)
        	...
        	@endif
        @endforeach
		<div class="input-group input-number" style="width: 120px; margin: 0 auto;">
			<span class="input-group-btn">
				<button type="button" class="btn btn-default btn-flat quantity-down"><i class="fa fa-minus text-danger"></i></button>
			</span>
			...
			>
			<span class="input-group-btn">
				<button type="button" class="btn btn-default btn-flat quantity-up"><i class="fa fa-plus text-success"></i></button>
			</span>
		</div>
		...
		<div class="text-center" style="margin-top: 3px;">
			@if(count($sub_units) > 1)
				<select name="products[{{$row_count}}][sub_unit_id]" class="form-control input-sm sub_unit inline-block" style="width: auto; height: 24px; padding: 2px 5px; font-size: 10px;">
					@foreach($sub_units as $key => $value)
			...
			@elseif(count($sub_units) == 1)
				...
				<span class="label label-default" style="font-size: 10px;">{{$_su['name']}}</span>
			@else
				<span class="label label-default" style="font-size: 10px;">{{$product->unit}}</span>
			@endif
		</div>
	...
	<td class="text-right" style="vertical-align: middle;">
		...
		<span class="display_currency pos_line_total_text @if(!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif" style="font-weight: bold; font-size: 13px;" data-currency_symbol="true">{{$product->quantity_ordered*$unit_price_inc_tax}}</span>
	</td>
	<td class="text-center" style="vertical-align: middle;">
		<i class="fa fa-close text-danger pos_remove_row cursor-pointer" style="font-size: 16px;"></i>
	</td>
</tr>
