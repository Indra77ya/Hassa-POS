...
<div id="{{ $filterId }}"
     class="tw-bg-white tw-border tw-border-gray-200 tw-mb-5 tw-overflow-hidden tw-duration-300 {{ $isOpen ? 'pf-open' : '' }}">

    {{-- Header --}}
    <button type="button"
            id="{{ $filterId }}_btn"
            class="pf-btn tw-w-full tw-flex tw-items-center tw-justify-between tw-px-5 tw-py-4 tw-border-0 tw-border-b tw-border-transparent tw-bg-transparent tw-cursor-pointer tw-transition-colors hover:tw-bg-gray-50/80 tw-outline-none focus:tw-outline-none"
        ...
        <span class="tw-flex tw-items-center tw-gap-3">
            {{-- Icon badge --}}
            <span class="tw-inline-flex tw-items-center tw-justify-center tw-shrink-0 tw-duration-300"
            ...
            {{-- Title --}}
            <span class="tw-text-sm tw-text-gray-800 tw-tracking-tight tw-transition-colors">
                {{ $title ?? __('report.filters') }}
            </span>
        </span>

        {{-- Chevron --}}
        <svg class="pf-chevron tw-w-5 tw-h-5 tw-shrink-0 tw-mr-2 tw-duration-300 {{ $isOpen ? 'tw-rotate-180' : '' }}"
