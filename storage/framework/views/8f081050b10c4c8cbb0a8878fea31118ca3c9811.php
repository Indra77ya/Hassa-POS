<script type="text/javascript">
	$(document).ready( function () {
	    ...
	    productions_table = $('#productions_table').DataTable({
	        ...
	        aaSorting: [[0, 'desc']],
	        ajax: {
	            url: '<?php echo e(action([\Modules\Manufacturing\Http\Controllers\ProductionController::class, 'index']), false); ?>',
	            "data": function ( d ) {
    	...
    	recipe_table = $('#recipe_table').DataTable({
	        processing: true,
	        serverSide: true,
	        ajax: '<?php echo e(action([\Modules\Manufacturing\Http\Controllers\RecipeController::class, 'index']), false); ?>',
	        columnDefs: [
	            {
	                targets: [0, 5, 6, 7],
	                orderable: false,
	...
	$(document).on('submit', '#recipe_form', function (e) {
		...
		if (ingredients_length < 1) {
			toastr.error('<?php echo app('translator')->get("manufacturing::lang.please_add_ingredients"); ?>');
			e.preventDefault();
			return false;
		}
	});
...
</script><?php /**PATH C:\laragon\www\Hassa POS\Modules\Manufacturing\Providers/../Resources/views/layouts/partials/common_script.blade.php ENDPATH**/ ?>